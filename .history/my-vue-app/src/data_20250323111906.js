...
const Game = () => {
  ...
  const CHARACTER_SPEED = TILE_SIZE; // Move exactly one tile at a time
  ...
  const chests = [
    { x: 4 * TILE_SIZE, y: 3 * TILE_SIZE, map: 'tilemap1', tileId: 36, contents: [{ id: 2, name: 'Iron Sword', description: 'A sturdy iron sword for close combat.' }] },
    { x: 14 * TILE_SIZE, y: 5 * TILE_SIZE, map: 'tilemap2', tileId: 2, contents: [{ id: 101, name: 'Health Potion', description: 'Restores 20 health when used.' }] },
    { x: 15 * TILE_SIZE, y: 4 * TILE_SIZE, map: 'tilemap3', tileId: 15, contents: [{ id: 37, name: 'Wooden Shield', description: 'A basic shield for defense.' }] },
    { x: 20 * TILE_SIZE, y: 4 * TILE_SIZE, map: 'tilemap3', tileId: 15, contents: [{ id: 37, name: 'Wooden Shield', description: 'A basic shield for defense.' }] },
  ];
    ...
    const handleMouseDown = (e) => {
      ...
      currentChestRef.current.contents.forEach((item, index) => {
        ...
        if (
          ...
          draggingItem.current = { item, index };
  ...
  useEffect(() => {
    ...
    const updateCharacter = (deltaTime) => {
      ...
      if (isMoving && isWalkable(newX, newY)) {
        ...
        characterRef.current.x = Math.round(characterRef.current.x / TILE_SIZE) * TILE_SIZE;
        characterRef.current.y = Math.round(characterRef.current.y / TILE_SIZE) * TILE_SIZE;
    ...
    const gameLoop = (timestamp) => {
      ...
      if (showChestRef.current) {
        drawChestScreen();
        drawInventoryScreen(); // Show inventory alongside chest
      }
