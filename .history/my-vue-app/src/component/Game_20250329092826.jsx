...
const Game = ({ onExitToMenu }) => {
  ...
  const [battleLog, setBattleLog] = useState([]); // New state for battle log
  ...
  const bossTurn = (boss) => {
    ...
    const damage = 15; // 3x original damage
    characterRef.current.health -= damage;
    setBattleLog(prev => [...prev.slice(-4), `Boss attacks you back`]); // Add to log, limit to 5 entries
    if (characterRef.current.health <= 0) {
      setBattleLog(prev => [...prev.slice(-4), 'Game Over! You died.']);
      setBattleMode(false);
      resetGameState();
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (key === 'i' && !battleModeRef.current) { // Disable inventory in battle mode
    ...
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          ...
          boss.health -= baseDamage; // Ensure health decreases
        ...
        if (hasDraught && clickX >= specialButton.x && clickX <= specialButton.x + specialButton.width && clickY >= specialButton.y && clickY <= specialButton.y + specialButton.height) {
          ...
          boss.health -= totalDamage; // Ensure health decreases
  ...
  useEffect(() => {
    ...
    const drawBattleLog = () => {
      ...
      const logX = (canvas.width - 300) / 2 - 220; // Left of battle menu
    ...
    const gameLoop = (timestamp) => {
      ...
      if (battleModeRef.current) {
        drawBattleMenu();
        drawBattleLog(); // Draw battle log next to menu
      }
    ...
    startGame();
  }, [battleLog]); // Add battleLog to dependencies
