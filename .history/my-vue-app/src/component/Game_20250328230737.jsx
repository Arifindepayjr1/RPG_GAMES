...
const Game = ({ onExitToMenu }) => {
  ...
  // Updated NPC data with spritesheets and frame info
  const npcs = [
    { 
      x: 18 * TILE_SIZE, 
      y: 4 * TILE_SIZE, 
      ...
      hasTalked: false, 
      spriteSrc: '/character1.png', // New spritesheet
      frameX: 0, 
      frameY: 0, 
      frameCount: 0, 
      maxFrames: 7 // 1 row, 7 columns
    },
    { 
      x: 5 * TILE_SIZE, 
      y: 6 * TILE_SIZE, 
      ...
      spriteSrc: '/character2.png', // New spritesheet
      frameX: 0, 
      frameY: 0, 
      frameCount: 0, 
      maxFrames: 7 // 1 row, 7 columns
    ...
    { 
      ...
      name: 'Rift Lord', 
      health: 100, 
      spriteSrc: '/final.png', // New spritesheet
      frameX: 0, 
      frameY: 0, 
      frameCount: 0, 
      maxFrames: 9 // 1 row, 9 columns
    },
  ];
  ...
  // Rest of your existing variables (items, chests, etc.) remain unchanged
  const items = {
  ...
  const chests = [
    { 
      ...
      map: 'tilemap1', 
      tileId: 36, 
      contents: [items[2], items[102], items[103]] 
    },
    { 
      x: 14 * TILE_SIZE, 
      y: 5 * TILE_SIZE, 
      map: 'tilemap2', 
      tileId: 2, 
      contents: [items[101], items[104], items[105]] 
    },
    { 
      x: 15 * TILE_SIZE, 
      y: 4 * TILE_SIZE, 
  ...
  const resetGameState = () => {
    ...
    npcs.forEach(npc => {
      ...
      npc.frameX = 0; // Reset NPC animation
      npc.frameCount = 0;
    });
  };
  ...
  useEffect(() => {
    ...
    const drawNPCs = (deltaTime) => {
      npcs.forEach(npc => {
        if (npc.map === currentMapRef.current && (npc.type === 'npc' || npc.health > 0)) {
          ...
          npc.frameCount += deltaTime;
          if (npc.frameCount >= SPRITE_ANIMATION_SPEED) {
            npc.frameX = (npc.frameX + 1) % npc.maxFrames;
            npc.frameCount = 0;
          }

          // Assuming NPC spritesheets have the same SPRITE_WIDTH, SPRITE_HEIGHT, BORDER_WIDTH, SPACING_WIDTH
          const srcX = npc.frameX * (SPRITE_WIDTH + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const srcY = npc.frameY * (SPRITE_HEIGHT + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const offsetX = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
          const offsetY = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
    ...
    const gameLoop = (timestamp) => {
      ...
      drawTilemap();
      drawNPCs(deltaTime); // Pass deltaTime to animate NPCs
