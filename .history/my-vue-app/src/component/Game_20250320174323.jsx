...
const Game = () => {
  ...
  const CHARACTER_SPEED = 4; // Increased from 3 to 4 (pixels per frame) for snappier movement
  ...
  useEffect(() => {
    ...
    const updateCharacter = () => {
      ...
      setCharacter(prev => {
        ...
        if (prev.isFighting && frameCount % 10 === 0) { // Fighting: every 10 frames (matches character.js)
          frameX = (frameX + 1) % 4;
          if (frameX === 0) return { ...prev, frameCount: 0, frameX, isFighting: false };
        } else if (isMoving && frameCount % 5 === 0) { // Walking: every 5 frames (matches character.js)
          frameX = (frameX + 1) % 4;
        } else if (!isMoving) {
          frameX = 0;
        }
