...
const Game = () => {
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === 'npc') {
        setDialogue(`[${nearbyNPC.name}] Press J to talk.`);
        if (keysRef.current.j) {
          ...
          if (nearbyNPC.name === 'Villager Anna') {
            if (inventory.some(item => item.id === 25)) {
              ...
              nearbyNPC.x = 22 * TILE_SIZE; // Move Anna to (22, 4)
        ...
        setDialogue(`[${nearbyNPC.name}] Fight me! Press J to attack.`);
        if (keysRef.current.j) {
          nearbyNPC.health -= 10;
          setDialogue(`You hit ${nearbyNPC.name}! Boss HP: ${nearbyNPC.health}`);
          
          if (nearbyNPC.health > 0) {
            characterRef.current.health -= 5;
            setDialogue(prev => `${prev}\n${nearbyNPC.name} hits back! Your HP: ${characterRef.current.health}`);
          } else {
            setDialogue(`${nearbyNPC.name} defeated!`);
            nearbyNPC.health = 0;
          }
  ...
  useEffect(() => {
      ...
      if (isMoving && isWalkable(newX, newY)) {
        ...
        if (overlayTile && !collectedItems.includes(`${tileY}-${tileX}`)) {
          if (overlayTile === 25) {
            ...
            setDialogue('You found Anna’s Ring!');
    ...
    const drawDialogue = () => {
      if (dialogue || inventory.length > 0) {
        if (dialogue) {
          ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
          ctx.fillRect(50, 400, 860, 100);
          ctx.fillStyle = 'white';
          ctx.font = '20px Arial';
          ctx.fillText(dialogue, 60, 440);
        }

        ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        ctx.fillRect(50, 520, 860, 60);
        ctx.fillStyle = 'white';
        ctx.font = '16px Arial';
        ctx.fillText('Inventory: ' + inventory.map(item => item.name).join(', '), 60, 550);
      }
    };
