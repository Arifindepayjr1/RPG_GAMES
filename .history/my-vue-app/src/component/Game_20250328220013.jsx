<<<<<<< HEAD:my-vue-app/src/component/Game.jsx
...
const Game = () => {
  ...
  const npcs = [
    ...
    },
    {
      ...
      itemToGive: {
        id: 2,
        name: "Crystal Sword",
        description: "A powerful sword forged to defeat the Rift Lord.",
      },
      frameX: 0,
      frameY: 0,
    },
  ...
  const items = {
    ...
    25: {
      ...
      description: "A glowing crystal needed to seal the rift.",
    },
    37: {
      id: 37,
      name: "Wooden Shield",
    ...
    101: {
      id: 101,
      name: "Health Potion",
      description: "Restores 20 health when used.",
    },
    102: {
      id: 102,
      name: "Leather Armor",
      description: "Light armor for basic protection.",
    },
    103: {
      id: 103,
      name: "Gold Coin",
      description: "A shiny coin worth 10 gold.",
    },
    104: {
      id: 104,
      name: "Mana Crystal",
      description: "Restores 15 mana when used.",
    },
    105: {
      id: 105,
      name: "Steel Dagger",
    ...
    106: {
      id: 106,
      name: "Fire Scroll",
      description: "Casts a fire spell when used.",
    },
    107: {
      id: 107,
      name: "Silver Ring",
      description: "Increases magic resistance by 5%.",
    },
  };
  ...
  const getCurrentMapData = () => {
    switch (currentMapRef.current) {
      case "tilemap1":
        return {
          map: map1,
          overlay: overlayLayer1,
          secondOverlay: secondOverlayLayer1,
          grassTile: 12,
        };
      case "tilemap2":
        return {
          map: map2,
          overlay: overlayLayer2,
          secondOverlay: secondOverlayLayer2,
          grassTile: 15,
        };
      case "tilemap3":
        return {
          map: map3,
          overlay: overlayLayer3,
          secondOverlay: secondOverlayLayer3,
          grassTile: 15,
        };
      default:
        return {
  ...
  const checkMapTransition = () => {
    ...
    if (currentTime - lastTransitionTimeRef.current < TRANSITION_COOLDOWN)
      return;
  ...
  const updateTransition = () => {
    ...
    if (!transitionFadeInRef.current) {
      transitionAlphaRef.current = Math.min(
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === "npc") {
        if (
          dialogueRef.current !== `[${nearbyNPC.name}] Press J to talk.` &&
          dialogueRef.current !== `${nearbyNPC.name}: ${nearbyNPC.dialogue}`
        ) {
          console.log(
            "Showing NPC prompt:",
            `[${nearbyNPC.name}] Press J to talk.`
          );
            ...
            if (nearbyNPC.name === "Mage Anna") {
              ...
              if (
                ...
                if (nearbyNPC.itemToGive) {
                  ...
                  nearbyNPC.dialogue =
                    "Go to the rift in the south and defeat the Rift Lord!";
                }
        ...
        if (
          dialogueRef.current !== `[${nearbyNPC.name}] Fight me!` &&
          dialogueRef.current !== `You attack the monster!` &&
          dialogueRef.current !== `Monster attacks back!` &&
          dialogueRef.current !== `${nearbyNPC.name} defeated!`
        ) {
        ...
        if (keysRef.current.j && !keysRef.current.jPressed) {
          ...
          const hasCrystalSword = inventoryRef.current.some(
            (item) => item.id === 2
          );
          ...
          setDialogueSync(
            `You attack the monster${
              hasCrystalSword ? " with the Crystal Sword" : ""
            }!`
          );
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (key === "i") {
        console.log(
          "I key pressed, current showInventory:",
          showInventory,
          "showChest:",
          showChest
        );
        ...
        setGamePaused((prev) => {
          ...
          console.log(
            "Setting gamePaused to:",
            newGamePaused,
            "newShowInventory:",
            newShowInventory,
            "showChest:",
            showChestRef.current
          );
      ...
      currentChestRef.current.contents.forEach((item, index) => {
        ...
        if (
          ...
          draggingOffset.current = {
            x: mouseX - itemBounds.x,
            y: mouseY - itemBounds.y,
          };
        }
      });
    };
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (
        ...
        clickX <=
          attackButtonBounds.current.x + attackButtonBounds.current.width &&
        clickY >= attackButtonBounds.current.y &&
        clickY <=
          attackButtonBounds.current.y + attackButtonBounds.current.height
      ) {
        const nearbyNPC = npcs.find(
          (npc) =>
            npc.map === currentMapRef.current &&
            npc.type === "boss" &&
            Math.abs(
              Math.floor(characterRef.current.x / TILE_SIZE) -
                Math.floor(npc.x / TILE_SIZE)
            ) <= 1 &&
            Math.abs(
              Math.floor(characterRef.current.y / TILE_SIZE) -
  ...
  useEffect(() => {
      ...
      for (let row = 0; row < 20; row++) {
        for (let col = 0; col < 30; col++) {
          ctx.drawImage(
            ...
            16,
            16,
            Math.round(col * TILE_SIZE),
            Math.round(row * TILE_SIZE),
            TILE_SIZE,
            TILE_SIZE
          );
        }
    ...
    const drawCharacter = (x, y, frameX, frameY) => {
      ...
      const srcX =
        frameX * (SPRITE_WIDTH + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
      const srcY =
        frameY * (SPRITE_HEIGHT + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
    ...
    const drawNPCs = () => {
      npcs.forEach((npc) => {
        if (
          npc.map === currentMapRef.current &&
          (npc.type === "npc" || npc.health > 0)
        ) {
          const spriteSheetToUse =
            npc.spriteSrc && npcSpriteSheets.current[npc.spriteSrc]
              ? npcSpriteSheets.current[npc.spriteSrc]
              : spriteSheet;
          ...
          const srcX =
            frameX * (SPRITE_WIDTH + BORDER_WIDTH + SPACING_WIDTH) +
            BORDER_WIDTH;
          const srcY =
            frameY * (SPRITE_HEIGHT + BORDER_WIDTH + SPACING_WIDTH) +
            BORDER_WIDTH;
          const offsetX = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
          const offsetY = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
    ...
    const updateCharacter = (deltaTime) => {
      ...
      if (
        ...
        characterRef.current.moveProgress < 1
      ) {
        characterRef.current.moveProgress +=
          CHARACTER_SPEED * (deltaTime / 16.67);
        if (characterRef.current.moveProgress >= 1) {
          ...
          characterRef.current.x =
            characterRef.current.x +
            (characterRef.current.targetX - characterRef.current.x) *
              characterRef.current.moveProgress;
          characterRef.current.y =
            characterRef.current.y +
            (characterRef.current.targetY - characterRef.current.y) *
              characterRef.current.moveProgress;
        }
      }
      ...
      if (
        characterRef.current.moveProgress > 0 ||
        currentTime - lastMoveTime < MOVE_COOLDOWN
      )
        return;
      ...
      if (
        characterRef.current.moving &&
        characterRef.current.frameCount >= SPRITE_ANIMATION_SPEED
      ) {
    ...
    const drawDialogue = () => {
      ...
      if (showAttackButton) {
        ...
        ctx.fillRect(
          attackButtonBounds.current.x,
          attackButtonBounds.current.y,
          attackButtonBounds.current.width,
          attackButtonBounds.current.height
        );
        ...
        ctx.fillText(
          "Attack (J)",
          attackButtonBounds.current.x + 30,
          attackButtonBounds.current.y + 28
        );
    ...
    const drawInventoryScreen = () => {
      ...
      if (!ctx) {
        console.error(
          "Canvas context (ctx) is not defined in drawInventoryScreen"
        );
        return;
      }
    ...
    const gameLoop = (timestamp) => {
      ...
      if (showInventoryRef.current) {
        console.log(
          "Drawing inventory screen, inventory:",
...
export default Game;
>>>>>>> 73292b14a96564c9c9d512cd7f609971a3ee2526:.history/my-vue-app/src/component/Game_20250328220013.jsx
