...
const Game = ({ onExitToMenu }) => {
  ...
  const characterRef = useRef({
    ...
    moveProgress: 0,
    powerBoost: 0,
    defending: false, // Added for Defend option
  });
  ...
  const [battleMode, setBattleMode] = useState(false); // New state for battle mode
  ...
  const battleModeRef = useRef(battleMode); // New ref for battle mode
  ...
  const items = {
    ...
    109: { id: 109, name: 'Draught of Might', description: 'Boosts attack power' },
  };

  const getRandomItems = () => {
    const possibleItems = [
      items[2], items[101], items[102], items[103], items[104], items[105], items[106], items[107],
      items[108], items[109],
    ];
  ...
  const bossTurn = (boss) => {
    ...
    const damage = characterRef.current.defending ? 2.5 : 5;
    characterRef.current.health -= damage;
    setDialogueSync(`Rift Lord attacks! You take ${damage} damage.`);
    characterRef.current.defending = false; // Reset defend after boss turn
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC && !battleModeRef.current) {
      if (nearbyNPC.type === 'npc') {
        ...
        setBattleMode(true);
        setDialogueSync('Battle with Rift Lord begins! Choose an action.');
        setShowAttackButton(false);
      }
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (isPausedRef.current) {
        ...
        if (clickX >= restartButton.x && clickX <= restartButton.x + restartButton.width && clickY >= restartButton.y && clickY <= restartButton.y + restartButton.height) {
          resetGameState();
        }

        const exitButton = { x: boxX + 20, y: boxY + 200, width: buttonWidth, height: buttonHeight };
        if (clickX >= exitButton.x && clickX <= exitButton.x + exitButton.width && clickY >= exitButton.y && clickY <= exitButton.y + exitButton.height) {
          setIsExiting(true);
          resetGameState();
          setTimeout(() => {
      ...
      if (battleModeRef.current) {
        const boss = npcs.find(npc => npc.type === 'boss' && npc.map === currentMapRef.current && npc.health > 0);
        const boxX = (canvasRef.current.width - 300) / 2;
        const boxY = 300;
        const buttonWidth = 260;
        const buttonHeight = 40;
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          ...
          const totalDamage = baseDamage + (characterRef.current.powerBoost || 0);
          boss.health -= totalDamage;
          setDialogueSync(`You attack Rift Lord for ${totalDamage} damage!`);
          if (boss.health <= 0) {
            setDialogueSync('Rift Lord defeated!');
            setBattleMode(false);
          } else {
            bossTurn(boss);
          }
        ...
        const useItemButton = { x: boxX + 20, y: boxY + 140, width: buttonWidth, height: buttonHeight };
        if (clickX >= useItemButton.x && clickX <= useItemButton.x + useItemButton.width && clickY >= useItemButton.y && clickY <= useItemButton.y + useItemButton.height) {
          setShowInventory(true);
          setGamePaused(true);
        }

        const defendButton = { x: boxX + 20, y: boxY + 200, width: buttonWidth, height: buttonHeight };
        if (clickX >= defendButton.x && clickX <= defendButton.x + defendButton.width && clickY >= defendButton.y && clickY <= defendButton.y + defendButton.height) {
          characterRef.current.defending = true;
          setDialogueSync('You brace for the next attack!');
          bossTurn(boss);
        }

        const fleeButton = { x: boxX + 20, y: boxY + 260, width: buttonWidth, height: buttonHeight };
        if (clickX >= fleeButton.x && clickX <= fleeButton.x + fleeButton.width && clickY >= fleeButton.y && clickY <= fleeButton.y + fleeButton.height) {
          const fleeSuccess = Math.random() > 0.5;
          if (fleeSuccess) {
            setBattleMode(false);
            setDialogueSync('You successfully fled the battle!');
          } else {
            setDialogueSync('Failed to flee!');
            bossTurn(boss);
          }
        }
      }

      if (showInventoryRef.current && !isPausedRef.current && !showChestRef.current) {
        const boxX = (canvasRef.current.width - 300) / 2;
        const boxY = 120;
        inventoryRef.current.forEach((item, index) => {
          ...
          if (clickX >= itemBounds.x && clickX <= itemBounds.x + itemBounds.width && clickY >= itemBounds.y && clickY <= itemBounds.y + itemBounds.height) {
            if (item.id === 108) {
              ...
              if (battleModeRef.current) {
                const boss = npcs.find(npc => npc.type === 'boss' && npc.map === currentMapRef.current);
                bossTurn(boss);
              }
            } else if (item.id === 109) {
  ...
  useEffect(() => {
    ...
    const updateCharacter = (deltaTime) => {
      if (transitioningRef.current || gamePausedRef.current || isPausedRef.current || battleModeRef.current) return;
    ...
    const drawBattleMenu = () => {
      ...
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Use Item', buttonX + buttonWidth / 2, boxY + 165);
      ...
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Defend', buttonX + buttonWidth / 2, boxY + 225);

      ctx.fillStyle = 'rgba(255, 165, 0, 0.7)';
      ctx.fillRect(buttonX, boxY + 260, buttonWidth, buttonHeight);
      ctx.strokeStyle = 'white';
      ctx.lineWidth = 2;
      ctx.strokeRect(buttonX, boxY + 260, buttonWidth, buttonHeight);
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Flee', buttonX + buttonWidth / 2, boxY + 285);
    ...
    const gameLoop = (timestamp) => {
      ...
      if (battleModeRef.current) drawBattleMenu();
      if (showInventoryRef.current) drawInventoryScreen();
      if (showChestRef.current) drawChestScreen();
