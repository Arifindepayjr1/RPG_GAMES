...
const Game = () => {
  ...
  const characterRef = useRef({
    ...
    health: 100,
    inventory: [],
    targetX: 5 * TILE_SIZE, // Target position for smooth movement
    targetY: 15 * TILE_SIZE,
    moveProgress: 0, // Progress of the current move (0 to 1)
  });
  ...
  const CHARACTER_SPEED = 0.1; // Speed of movement progress per frame (0 to 1)
  const SPRITE_ANIMATION_SPEED = 8;
  const MOVE_COOLDOWN = 200; // Cooldown in milliseconds between moves
  let lastMoveTime = 0;
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp) => {
      ...
      drawTilemap();
      drawNPCs();
      updateCharacter(deltaTime); // Always update character for rendering purposes
