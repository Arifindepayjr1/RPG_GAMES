...
const Game = () => {
  ...
  const autoplayFailedRef = useRef(autoplayFailed);
  ...
  useEffect(() => {
    ...
    console.log('gameStarted updated to:', gameStarted); // Added logging
  }, [gameStarted]);

  useEffect(() => {
    musicPlayingRef.current = musicPlaying;
    console.log('musicPlaying updated to:', musicPlaying); // Added logging
  }, [musicPlaying]);

  useEffect(() => {
    autoplayFailedRef.current = autoplayFailed;
  }, [autoplayFailed]);
  ...
  useEffect(() => {
    soundtrackRef.current = new Audio('/audio/menu-soundtrack.mp3');
    ...
    const playSoundtrack = () => {
      if (musicPlaying && !gameStarted) {
        soundtrackRef.current.play()
          .then(() => {
            console.log('Soundtrack autoplay successful');
  ...
  // Play or pause the soundtrack based on the musicPlaying and gameStarted states
  useEffect(() => {
    if (!soundtrackRef.current) return;

    if (musicPlaying && !gameStarted) {
      console.log('Attempting to play soundtrack: musicPlaying=', musicPlaying, 'gameStarted=', gameStarted);
      soundtrackRef.current.play()
        .then(() => {
      ...
      console.log('Pausing soundtrack: musicPlaying=', musicPlaying, 'gameStarted=', gameStarted);
      soundtrackRef.current.pause();
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (!gameStartedRef.current) {
        ...
        // If autoplay failed, any click should attempt to play the audio
        if (autoplayFailedRef.current) {
          console.log('Autoplay failed, user clicked to enable audio');
  ...
  useEffect(() => {
    ...
    const drawMenu = () => {
      ...
      // Autoplay failed prompt
      if (autoplayFailedRef.current) {
        ctx.fillStyle = 'rgba(255, 0, 0, 0.8)';
        ctx.fillRect(300, 450, 360, 50);
        ctx.strokeStyle = 'white';
        ctx.lineWidth = 2;
        ctx.strokeRect(300, 450, 360, 50);
        ctx.fillStyle = 'white';
        ctx.font = '18px Arial';
        ctx.fillText('Click anywhere to enable audio', canvas.width / 2, 480);
      }
