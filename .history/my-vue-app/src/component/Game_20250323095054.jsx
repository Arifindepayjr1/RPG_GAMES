...
const Game = () => {
  ...
  const SPRITE_ANIMATION_SPEED = 10; // Adjust this to change sprite animation speed

  const npcs = [
    { x: 18 * TILE_SIZE, y: 4 * TILE_SIZE, map: 'tilemap1', type: 'npc', name: 'Mage Anna', dialogue: 'The rift is growing! I need a magical crystal from the western cave to seal it. Can you help?', hasTalked: false, frameY: 3 },
    { x: 5 * TILE_SIZE, y: 6 * TILE_SIZE, map: 'tilemap1', type: 'npc', name: 'Blacksmith Ben', dialogue: 'Greetings! Have you spoken to Mage Anna? She needs a crystal to seal the rift.', hasTalked: false, itemToGive: { id: 2, name: 'Crystal Sword' }, frameY: 4 },
    { x: 17 * TILE_SIZE, y: 12 * TILE_SIZE, map: 'tilemap3', type: 'boss', name: 'Rift Lord', health: 100, frameY: 5 },
  ];
    ...
    const drawNPCs = () => {
      npcs.forEach(npc => {
        if (npc.map === currentMapRef.current && (npc.type === 'npc' || npc.health > 0)) {
          ...
          const srcX = 0 * (SPRITE_WIDTH + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const srcY = frameY * (SPRITE_HEIGHT + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const offsetX = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
          const offsetY = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
