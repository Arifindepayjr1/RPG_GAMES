  ...
  useEffect(() => {
    ...
    const drawInventoryScreen = () => {
      ...
      if (inventory.length === 0) {
        ctx.fillText('Inventory is empty (Press I to Close)', boxX + boxWidth / 2, boxY + boxHeight / 2);
      } else {
        ctx.fillText('Inventory (Press I to Close)', boxX + boxWidth / 2, boxY + 40);
        ctx.textAlign = 'left';

        inventory.forEach((item, index) => {
          const yPos = boxY + 80 + index * 60;
          ...
          ctx.fillRect(boxX + 20, yPos - 15, 40, 40);
          ctx.strokeStyle = 'white';
          ctx.lineWidth = 1;
          ctx.strokeRect(boxX + 20, yPos - 15, 40, 40);

          ctx.fillStyle = '#F0E68C';
          ctx.font = '18px Arial';
          ctx.fillText(item.name, boxX + 70, yPos);
          ...
          ctx.fillText(description, boxX + 70, yPos + 20);
        });
      }
    };
    ...
    const gameLoop = (timestamp) => {
      ...
      if (showInventory) {
        console.log('Drawing inventory screen, inventory:', inventory);
        drawInventoryScreen();
      }
