...
const Game = ({ onExitToMenu }) => {
  ...
  const getRandomItems = () => {
    ...
    const includeExtraElixir = Math.random() < 0.05; // Increased from 2% to 5%
    if (includeExtraElixir) {
      selectedItems.push(items[108]);
    }

    // Add a 5% chance to include an additional Health Potion (ID 101)
    const includeExtraHealthPotion = Math.random() < 0.05; // 5% chance
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e) => {
      ...
      const inventoryBoxHeight = 450; // Updated to match chest height
  ...
  useEffect(() => {
    ...
    const drawInventoryScreen = () => {
      ...
      const boxWidth = 300;
      const boxHeight = 450; // Updated to match chest height (was 400)
