...
const Game = () => {
  ...
  const keysRef = useRef({ w: false, s: false, a: false, d: false, j: false }); // Add 'j' for attack
  const currentMapRef = useRef('tilemap1');
  const transitioningRef = useRef(false);
  const transitionAlphaRef = useRef(0);
  const transitionFadeInRef = useRef(false);
  const lastTransitionTimeRef = useRef(0);
  const [dialogue, setDialogue] = useState(''); // For NPC interaction and combat text
  ...
  // Define NPCs and Boss
  const npcs = [
    { x: 11 * TILE_SIZE, y: 4 * TILE_SIZE, map: 'tilemap1', type: 'npc' }, // NPC HERE (row 4, col 11)
  ...
  const isWalkable = (newX, newY) => {
    ...
    // Check if position is occupied by an NPC or boss
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === 'npc') {
        setDialogue('Press J to talk to the NPC.');
        if (keysRef.current.j) {
          setDialogue('Hello, traveler! Welcome to this world.');
        }
      } else if (nearbyNPC.type === 'boss') {
        setDialogue('Boss: Prepare to fight! Press J to attack.');
        if (keysRef.current.j) {
          // Player attacks boss
          nearbyNPC.health -= 10;
          ...
          if (nearbyNPC.health > 0) {
            ...
            setDialogue(prev => `${prev}\nBoss hits you! Your HP: ${characterRef.current.health}`);
          } else {
            setDialogue('You defeated the boss!');
            nearbyNPC.health = 0; // Boss defeated
          }

          // Reset J key to prevent continuous attack
    ...
    if (characterRef.current.health <= 0) {
      setDialogue('Game Over! You were defeated.');
    }
  };
  ...
  useEffect(() => {
    ...
    const drawCharacter = (x, y, frameX = 0, frameY = 2) => {
