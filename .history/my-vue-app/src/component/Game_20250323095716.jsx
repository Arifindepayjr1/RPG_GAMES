...
const Game = () => {
  ...
  const [displayedDialogue, setDisplayedDialogue] = useState(''); // For typewriter effect
  const fullDialogueRef = useRef(''); // Full dialogue text
  const dialogueIndexRef = useRef(0); // Current index for typewriter effect
  const isTypingRef = useRef(false); // Flag for typing in progress
  ...
  useEffect(() => {
    ...
    const drawNPCs = () => {
      npcs.forEach(npc => {
        if (npc.map === currentMapRef.current && (npc.type === 'npc' || npc.health > 0)) {
          ...
          const frameX = npc.frameX !== undefined ? npc.frameX : 0; // Use npc.frameX
          const frameY = npc.frameY !== undefined ? npc.frameY : 2; // Use npc.frameY
          const srcX = frameX * (SPRITE_WIDTH + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const srcY = frameY * (SPRITE_HEIGHT + BORDER_WIDTH + SPACING_WIDTH) + BORDER_WIDTH;
          const offsetX = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
          const offsetY = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2;
    ...
    const drawHealthBars = () => {
      ...
      ctx.fillRect(50, 50, 200, 30); // Background
      ctx.fillStyle = 'red';
      const playerHealthWidth = (characterRef.current.health / 100) * 180;
      ctx.fillRect(60, 60, playerHealthWidth, 10); // Health bar fill
      ctx.fillStyle = 'white';
      ctx.font = '16px Arial';
      ctx.fillText(`Player HP: ${characterRef.current.health}`, 60, 75); // This line displays the player's health as text

      // Boss health bar (only when near the boss)
      ...
      if (nearbyBoss && nearbyBoss.health > 0) {
        ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        ctx.fillRect(710, 50, 200, 30); // Background
        ctx.fillStyle = 'red';
        const bossHealthWidth = (nearbyBoss.health / 100) * 180;
        ctx.fillRect(720, 60, bossHealthWidth, 10); // Health bar fill
        ctx.fillStyle = 'white';
        ctx.font = '16px Arial';
        ctx.fillText(`Boss HP: ${nearbyBoss.health}`, 720, 75); // Boss health text
      }
    };
