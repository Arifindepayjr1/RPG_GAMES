    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === 'npc') {
        ...
        if (keysRef.current.j && !keysRef.current.jPressed) {
          ...
          const hasCrystalSword = inventory.some(item => item.id === 2);
  ...
  useEffect(() => {
    ...
    const drawDialogue = () => {
      ...
      if (inventory.length > 0) {
        const inventoryBoxHeight = 30 + inventory.length * 25;
        ctx.fillStyle = 'rgba(20, 30, 60, 0.9)';
        ctx.fillRect(50, 400 + dialogueBoxHeight, 860, inventoryBoxHeight);
        ctx.strokeStyle = 'gold';
        ctx.lineWidth = 2;
        ctx.strokeRect(50, 400 + dialogueBoxHeight, 860, inventoryBoxHeight);

        ctx.fillStyle = '#F0E68C';
        ctx.font = '16px Arial';
        ctx.fillText('Inventory:', 70, 430 + dialogueBoxHeight);

        inventory.forEach((item, index) => {
          const yPos = 455 + dialogueBoxHeight + index * 25;
          ...
          ctx.strokeStyle = 'white';
          ctx.lineWidth = 1;
          ctx.strokeRect(70, yPos - 15, 20, 20);

          ctx.fillStyle = '#F0E68C';
          ctx.fillText(item.name, 100, yPos);
        });
      }
    };
