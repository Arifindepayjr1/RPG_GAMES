...
const Game = ({ onExitToMenu }) => {
  ...
  const [actionCooldown, setActionCooldown] = useState(false); // New state for turn delay
  ...
  const actionCooldownRef = useRef(actionCooldown); // Ref for cooldown
  ...
  const bossTurn = (boss) => {
    ...
    const damage = 15; // 3x original damage (was 5)
  ...
  const checkInteraction = () => {
    ...
    if (nearbyChest && !battleModeRef.current) {
      ...
      if (keysRef.current.j && !keysRef.current.jPressed) {
        ...
        setShowInventory(true); // Ensure inventory is visible with chest
        setGamePaused(true);
        setDialogueSync('');
      }
      return;
    ...
    if (nearbyNPC && !battleModeRef.current) {
      if (nearbyNPC.type === 'npc') {
        ...
        setBattleMode(true);
        setDialogueSync(''); // Clear dialogue on battle start
        setShowAttackButton(false);
      }
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (key === 'i') { // Allow inventory during battle mode
  ...
  const resetGameState = () => {
    ...
    setIsPaused(false);
    setBattleMode(false);
    setActionCooldown(false); // Reset cooldown
    ...
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          const hasprinciplehasCrystalSword = inventoryRef.current.some(item => item.id === 2);
        ...
        if (clickX >= killMyselfButton.x && clickX <= killMyselfButton.x + killMyselfButton.width && clickY >= killMyselfButton.y && clickY <= killMyselfButton.y + killMyselfButton.height) {
          resetGameState();
          setBattleMode(false);
          setDialogueSync('You killed yourself. Game restarted.');
        }
      }
  ...
  useEffect(() => {
    ...
    const updateCharacter = (deltaTime) => {
      if (transitioningRef.current || gamePausedRef.current || isPausedRef.current || battleModeRef.current) return; // Disable movement in battle mode
    ...
    const gameLoop = (timestamp) => {
      ...
      drawHealthBars();
      drawDialogue();
      drawChestScreen(); // Draw chest first
      drawInventoryScreen(); // Draw inventory over chest
