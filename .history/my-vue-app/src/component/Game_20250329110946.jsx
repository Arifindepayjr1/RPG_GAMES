...
const Game = ({ onExitToMenu }) => {
  ...
  const npcs = [
    ...
    { x: 17 * TILE_SIZE, y: 12 * TILE_SIZE, map: 'tilemap3', type: 'boss', name: 'Rift Lord', health: 100, frameX: 0, frameY: 0, defeated: false },
  ];
    ...
    const handleKeyDown = (e) => {
      ...
      if (key === 'i' && !battleModeRef.current) {
        ...
        if (showChest) {
          ...
          setShowInventory(false); // Ensure inventory is closed
          setGamePaused(false); // Explicitly set to false
        } else {
          setGamePaused(prev => !prev); // Toggle gamePaused only if not closing a chest
        }
      }
      if (key === 'escape') {
    ...
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          ...
          boss.health = Math.max(boss.health - baseDamage, 0);
          console.log(`Boss health after attack: ${boss.health}`); // Debug log
        ...
        if (hasDraught && clickX >= specialButton.x && clickX <= specialButton.x + specialButton.width && clickY >= specialButton.y && clickY <= specialButton.y + specialButton.height) {
          ...
          boss.health = Math.max(boss.health - totalDamage, 0);
          console.log(`Boss health after special attack: ${boss.health}`); // Debug log
  ...
  useEffect(() => {
    ...
    const drawHealthBars = () => {
      ...
      if (boss && battleModeRef.current) {
        console.log(`Drawing boss health: ${boss.health}`); // Debug log
        ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
