...
const Game = ({ onExitToMenu }) => {
  ...
  const [isExiting, setIsExiting] = useState(false); // State for exiting
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (key === 'escape') {
        ...
        // Close inventory and chest if open
        if (showInventory) {
        ...
        setGamePaused(false); // Ensure gamePaused is false when pausing
      }
    };
  ...
  useEffect(() => {
    ...
    const drawHealthBars = () => {
      ...
      ctx.fillRect(600, 590, playerHealthWidth, 10);
      ctx.fillStyle = 'white';
      ctx.font = '16px Arial';
