...
const Game = ({ onExitToMenu }) => {
  ...
  const characterRef = useRef({
    ...
    moveProgress: 0,
    powerBoost: 0, // Added for Draught of Might
  });
  ...
  const items = {
    ...
    108: { id: 108, name: 'Elixir of Vitality', description: 'Restores 20 health when consumed.' }, // New item
    109: { id: 109, name: 'Draught of Might', description: 'Boosts attack power by 5 for 30 seconds.' }, // New item
  };

  // Helper function to randomize chest contents
  const getRandomItems = () => {
    const possibleItems = [
      items[2], items[101], items[102], items[103], items[104], items[105], items[106], items[107],
      items[108], items[109] // Include new items
    ];
    const numItems = Math.floor(Math.random() * 3) + 1; // 1-3 items per chest
    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === 'npc') {
        ...
        if (keysRef.current.j && !keysRef.current.jPressed) {
          ...
          const totalDamage = baseDamage + (characterRef.current.powerBoost || 0); // Apply power boost
          nearbyNPC.health -= totalDamage;
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (key === 'escape') {
        console.log('Escape key pressed, toggling pause state');
        setIsPaused(prev => {
          const newPausedState = !prev;
          console.log('isPaused toggled to:', newPausedState);
  ...
  const resetGameState = () => {
    characterRef.current = {
      ...
      targetY: 15 * TILE_SIZE,
      moveProgress: 0,
      powerBoost: 0, // Reset power boost
    };
    setInventory([]);
    setCollectedItems([]);
    setShowAttackButton(false);
    ...
    const handleCanvasClick = (e) => {
      ...
      if (showInventoryRef.current && !isPausedRef.current && !showChestRef.current) {
        ...
        inventoryRef.current.forEach((item, index) => {
          ...
          if (
            clickX >= itemBounds.x &&
            clickX <= itemBounds.x + itemBounds.width &&
            clickY >= itemBounds.y &&
            clickY <= itemBounds.y + itemBounds.height
          ) {
            if (item.id === 108) { // Elixir of Vitality
            ...
            } else if (item.id === 109) { // Draught of Might
  ...
  useEffect(() => {
    ...
    const drawPauseMenu = () => {
      if (!ctx) {
        console.error('Canvas context (ctx) is not defined in drawPauseMenu');
        return;
      }
