...
const Game = ({ onExitToMenu }) => {
  ...
  const getRandomItems = () => {
    ...
    // Add a 5% chance to include an additional Elixir of Vitality (ID 108)
    const includeExtraElixir = Math.random() < 0.05;
    if (includeExtraElixir) {
      selectedItems.push(items[108]);
    }

    // Add a 5% chance to include an additional Health Potion (ID 101)
    const includeExtraHealthPotion = Math.random() < 0.05;
    if (includeExtraHealthPotion) {
      selectedItems.push(items[101]);
    }
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        const hasHealthPotion = inventoryRef.current.some(item => item.id === 101);
        if ((hasElixir || hasHealthPotion) && characterRef.current.health < 90 && clickX >= healButton.x && clickX <= healButton.x + healButton.width && clickY >= healButton.y && clickY <= healButton.y + healButton.height) {
          // Prioritize Elixir of Vitality (ID 108) if available
          if (hasElixir) {
            ...
            const healthPotionIndex = inventoryRef.current.findIndex(item => item.id === 101);
            characterRef.current.health = Math.min(characterRef.current.health + 20, 100);
            setInventory(prev => prev.filter((_, i) => i !== healthPotionIndex));
            setBattleLog(prev => [...prev.slice(-4), `You heal yourself with Health Potion`]);
          }
        }
        ...
        inventoryRef.current.forEach((item, index) => {
          ...
          if (clickX >= itemBounds.x && clickX <= itemBounds.x + itemBounds.width && clickY >= itemBounds.y && clickY <= itemBounds.y + itemBounds.height) {
            if (item.id === 108) {
              ...
              characterRef.current.health = Math.min(characterRef.current.health + 20, 100);
              setInventory(prev => prev.filter((_, i) => i !== index));
              setDialogueSync('Used Health Potion: +20 HP');
            } else if (item.id === 109) {
  ...
  useEffect(() => {
    ...
    const drawBattleMenu = () => {
      ...
      const hasElixir = inventoryRef.current.some(item => item.id === 108);
      const hasHealthPotion = inventoryRef.current.some(item => item.id === 101);
      ctx.fillStyle = (hasElixir || hasHealthPotion) && characterRef.current.health < 90 ? 'rgba(0, 128, 0, 0.7)' : 'rgba(128, 128, 128, 0.7)';
