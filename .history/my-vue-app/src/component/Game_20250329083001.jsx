...
const Game = ({ onExitToMenu }) => {
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current) {
        ...
        const boxX = (canvasRef.current.width - 300) / 2;
        const boxY = (canvasRef.current.height - 300) / 2; // Centered vertically
        const buttonWidth = 260;
        const buttonHeight = 40;
        ...
        if (hasElixir && clickX >= healButton.x && clickX <= healButton.x + healButton.width && clickY >= healButton.y && clickY <= healButton.y + healButton.height) {
        ...
        if (hasDraught && clickX >= specialButton.x && clickX <= specialButton.x + specialButton.width && clickY >= specialButton.y && clickY <= specialButton.y + specialButton.height) {
          const hasCrystalSword = inventoryRef.current.some(item => item.id === 2);
          const baseDamage = hasCrystalSword ? 15 : 10;
          const totalDamage = (baseDamage + (characterRef.current.powerBoost || 0)) * 2;
  ...
  useEffect(() => {
    ...
    const drawBattleMenu = () => {
      ...
      // Heal
      const hasElixir = inventoryRef.current.some(item => item.id === 108);
      ctx.fillStyle = hasElixir ? 'rgba(0, 128, 0, 0.7)' : 'rgba(128, 128, 128, 0.7)';
      ...
      // Special
      const hasDraught = inventoryRef.current.some(item => item.id === 109);
      ctx.fillStyle = hasDraught ? 'rgba(0, 0, 255, 0.7)' : 'rgba(128, 128, 128, 0.7)';
