...
const Game = () => {
  ...
  const keysRef = useRef({ w: false, s: false, a: false, d: false, j: false, jPressed: false, space: false, spacePressed: false });
  ...
  const setDialogueSync = (text, skip = false) => {
    ...
    if (skip) {
      ...
      dialogueIndexRef.current = fullDialogueRef.current.length;
      isTypingRef.current = false;
      dialogueRef.current = fullDialogueRef.current;
      console.log('Skipped typing, showing full dialogue:', fullDialogueRef.current);
      return;
    }
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC) {
      if (nearbyNPC.type === 'npc') {
        ...
        if (keysRef.current.j && !keysRef.current.jPressed) {
          ...
          }, dialogueText.length * 20 + 2000);
        }

        if (keysRef.current.space && !keysRef.current.spacePressed && isTypingRef.current && dialogueRef.current !== `[${nearbyNPC.name}] Press J to talk.`) {
          console.log('Space pressed during typing, skipping to full dialogue');
          keysRef.current.spacePressed = true;
          setDialogueSync(dialogueRef.current, true);
          if (dialogueTimerRef.current) clearTimeout(dialogueTimerRef.current);
          dialogueTimerRef.current = setTimeout(() => {
            if (!isTypingRef.current) {
              console.log('Dialogue timer finished, resetting prompt for:', nearbyNPC.name);
        ...
        if (keysRef.current.j && !keysRef.current.jPressed) {
          ...
          dialogueTimerRef.current = setTimeout(() => {
            if (nearbyNPC.health > 0) {
              ...
              }, 2000 + 20 * `Monster attacks back!`.length);
            } else {
              setDialogueSync(`${nearbyNPC.name} defeated!`);
              nearbyNPC.health = 0;
              setShowAttackButton(false);
            }
          }, 2000 + 20 * `You attack the monster!`.length);
        }

        if (keysRef.current.space && !keysRef.current.spacePressed && isTypingRef.current && dialogueRef.current !== `[${nearbyNPC.name}] Fight me!`) {
          console.log('Space pressed during typing, skipping to full dialogue');
          keysRef.current.spacePressed = true;
          setDialogueSync(dialogueRef.current, true);
        }
    ...
    if (!keysRef.current.space) {
      keysRef.current.spacePressed = false;
    }
  };
  ...
  useEffect(() => {
    ...
    const drawDialogue = () => {
      ...
      if (displayedDialogue) {
        ...
        if (isTypingRef.current && !displayedDialogue.includes('Press J to talk')) {
          ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
          ctx.fillRect(50, 500, 860, 30);
          ctx.fillStyle = 'white';
          ctx.font = '16px Arial';
          ctx.fillText('Press J to talk / Space to skip', 60, 520);
          dialogueBoxHeight += 30;
        }
      }
