...
const Game = () => {
  ...
  const CHARACTER_SPEED = 3; // Reduced from 4 to 3
  const SPRITE_ANIMATION_SPEED = 8; // Increased from 6 to 8
  ...
  useEffect(() => {
    ...
    const drawDialogue = () => {
      ...
      if (inventory.length > 0) {
        ...
        ctx.strokeStyle = 'gold';
        ctx.lineWidth = 2;
        ctx.strokeRect(50, 400 + dialogueBoxHeight, 860, 60);
        ctx.fillStyle = '#F0E68C';
        ctx.font = '16px Arial';
        ctx.fillText('Inventory: ' + inventory.map(item => item.name).join(', '), 70, 430 + dialogueBoxHeight);
      }
    };
