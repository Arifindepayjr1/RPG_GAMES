    ...
    const handleKeyDown = (e) => {
      ...
      if (key === 'i' && !battleModeRef.current) {
        ...
        if (showChest) {
          ...
          setGamePaused(false); // Ensure game unpauses when closing chest
        }
      ...
      if (key === 'escape') {
        if (battleModeRef.current) {
          ...
          if (showChest) {
            setShowChest(false);
            setCurrentChest(null);
            setGamePaused(false); // Ensure game unpauses
          }
          setGamePaused(false);
        }
      }
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          ...
          if (boss.health <= 0) {
            ...
            setBattleMode(false); // Exit battle mode immediately
  ...
  useEffect(() => {
    ...
    const drawHealthBars = () => {
      ...
      if (nearbyBoss && nearbyBoss.health > 0) {
        ...
        ctx.fillStyle = 'red';
        const bossHealthWidth = Math.max((nearbyBoss.health / 100) * 180, 0); // Ensure no negative width
        ctx.fillRect(720, 590, bossHealthWidth, 10);
        ctx.fillStyle = 'white';
        ctx.font = '16px Arial';
        ctx.fillText(`Boss HP: ${Math.max(nearbyBoss.health, 0)}`, 720, 605); // Show 0 if health < 0
      }
    };
