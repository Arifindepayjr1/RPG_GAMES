...
const Game = ({ onExitToMenu }) => {
  ...
  const npcs = [
    ...
    { x: 17 * TILE_SIZE, y: 12 * TILE_SIZE, map: 'tilemap3', type: 'boss', name: 'Rift Lord', health: 100, frameX: 0, frameY: 0, defeated: false }, // Added defeated flag
  ];
  ...
  const bossTurn = (boss) => {
    ...
    if (characterRef.current.health <= 0) {
      setBattleLog(prev => [...prev.slice(-4), 'You die, restart the game']); // Updated log message
      setBattleMode(false);
      resetGameState();
    }
  };
  ...
  const checkInteraction = () => {
    ...
    if (nearbyNPC && !battleModeRef.current) {
      ...
      } else if (nearbyNPC.type === 'boss' && !nearbyNPC.defeated) { // Only enter battle if not defeated
        setBattleMode(true);
        setDialogueSync('');
        setShowAttackButton(false);
      }
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e) => {
      ...
      if (
        ...
        // Only show dialogue for Elixir of Vitality, not Draught of Might
        if (item.id === 108) {
          setDialogueSync('Picked up Elixir of Vitality');
        }
      }
  ...
  const resetGameState = () => {
    ...
    npcs.forEach(npc => {
      if (npc.type === 'npc') {
        ...
        npc.health = 100;
        npc.defeated = false; // Reset defeated flag
      }
    });
  };
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        const boxX = (canvasRef.current.width - 300) / 2;
        const boxY = (canvasRef.current.height - 300) / 2;
        const buttonWidth = 260;
        const buttonHeight = 40;
        ...
        if (clickX >= attackButton.x && clickX <= attackButton.x + attackButton.width && clickY >= attackButton.y && clickY <= attackButton.y + attackButton.height) {
          ...
          boss.health = Math.max(boss.health - baseDamage, 0); // Ensure health doesn’t go below 0
          setBattleLog(prev => [...prev.slice(-4), `You attack the boss`]);
          if (boss.health <= 0) {
            boss.defeated = true; // Mark boss as defeated
        ...
        if (hasElixir && characterRef.current.health < 90 && clickX >= healButton.x && clickX <= healButton.x + healButton.width && clickY >= healButton.y && clickY <= healButton.y + healButton.height) {
          ...
          setBattleLog(prev => [...prev.slice(-4), `You heal yourself`]);
        }
        ...
        if (hasDraught && clickX >= specialButton.x && clickX <= specialButton.x + specialButton.width && clickY >= specialButton.y && clickY <= specialButton.y + specialButton.height) {
          const hasCrystalSword = inventoryRef.current.some(item => item.id === 2);
          const baseDamage = hasCrystalSword ? 15 : 10;
          const totalDamage = baseDamage * 2;
          boss.health = Math.max(boss.health - totalDamage, 0); // Ensure health doesn’t go below 0
  ...
  useEffect(() => {
    ...
    const drawNPCs = () => {
      npcs.forEach(npc => {
        if (npc.map === currentMapRef.current && (npc.type === 'npc' || (npc.type === 'boss' && !npc.defeated))) { // Only draw undefeated bosses
    ...
    const drawHealthBars = () => {
      ...
      const boss = npcs.find(npc => 
        npc.type === 'boss' && 
        npc.map === currentMapRef.current && 
        !npc.defeated
      );
      if (boss && battleModeRef.current) { // Only show during battle
        ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        ctx.fillRect(710, 580, 200, 30);
        ctx.fillStyle = 'red';
        const bossHealthWidth = Math.max((boss.health / 100) * 180, 0);
        ctx.fillRect(720, 590, bossHealthWidth, 10);
        ctx.fillStyle = 'white';
        ctx.font = '16px Arial';
        ctx.fillText(`Boss HP: ${Math.max(boss.health, 0)}`, 720, 605);
      }
    };
    ...
    const drawBattleMenu = () => {
      ...
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Heal', buttonX + buttonWidth / 2, boxY + 165);
      ...
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Special', buttonX + buttonWidth / 2, boxY + 225);
      ...
      ctx.fillStyle = 'white';
      ctx.font = '18px Arial';
      ctx.fillText('Kill Myself', buttonX + buttonWidth / 2, boxY + 285);
    ...
    const drawBattleLog = () => {
      ...
      const logY = (canvas.height - 300) / 2;
      const logWidth = 200;
      const logHeight = 300;
