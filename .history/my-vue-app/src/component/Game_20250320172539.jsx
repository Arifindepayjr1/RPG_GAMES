...
const Game = () => {
  ...
  const CHARACTER_SPEED = 200; // Pixels per second (was 10 per frame, now time-based)
  ...
  useEffect(() => {
    ...
    const updateCharacter = (deltaTime) => {
      ...
      const speed = CHARACTER_SPEED * deltaTime; // Adjust speed by time elapsed
      ...
      setCharacter(prev => {
        const frameCount = prev.frameCount + deltaTime * 60; // Scale frame count by 60 FPS base
        let frameX = prev.frameX;
        if (prev.isFighting && frameCount >= 10) {
          frameX = (frameX + 1) % 4;
          if (frameX === 0) return { ...prev, frameCount: 0, frameX, isFighting: false };
        } else if (isMoving && frameCount >= 3) { // Faster animation (every 3 "virtual frames")
          frameX = (frameX + 1) % 4;
