...
const Game = ({ onExitToMenu }) => {
  ...
  const getRandomItems = () => {
    ...
    // Add a 2% chance to include an additional Elixir of Vitality
    const includeExtraElixir = Math.random() < 0.02; // 2% chance
  ...
  useEffect(() => {
    const handleMouseDown = (e) => {
      ...
      const chestBoxHeight = 450; // Increased height to match drawChestScreen
  ...
  useEffect(() => {
    const handleCanvasClick = (e) => {
      ...
      if (battleModeRef.current && !actionCooldownRef.current) {
        ...
        const boxX = (canvasRef.current.width - 300) / 2;
        const boxY = (canvasRef.current.height - 350) / 2; // Adjusted for increased boxHeight
        const buttonWidth = 260;
        const buttonHeight = 40;
  ...
  useEffect(() => {
    ...
    const drawBattleMenu = () => {
      ...
      const boxWidth = 300;
      const boxHeight = 350; // Increased height by 50 pixels
      ...
      const buttonSpacing = 60; // Increased spacing between buttons
    ...
    const drawBattleLog = () => {
      ...
      const logY = (canvas.height - 350) / 2; // Adjusted for increased boxHeight
      const logWidth = 200;
      const logHeight = 350;
    ...
    const drawChestScreen = () => {
      ...
      const boxWidth = 300;
      const boxHeight = 450; // Increased height by 50 pixels
      const boxX = 180;
      const boxY = 120;
