...
const Game = () => {
  ...
  const CHARACTER_SPEED = 0.15; // Increased from 0.1 to 0.15 for faster movement
  const SPRITE_ANIMATION_SPEED = 8;
  const MOVE_COOLDOWN = 150; // Decreased from 200ms to 150ms
  let lastMoveTime = 0;
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (key === 'i') {
        ...
        setGamePaused(prev => {
          const newShowInventory = !showInventoryRef.current; // Use the ref to get the current state before the update
