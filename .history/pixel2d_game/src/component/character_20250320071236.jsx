...
import { SPRITE_WIDTH, SPRITE_HEIGHT, BORDER_WIDTH, SPACING_WIDTH, CHARACTER_DISPLAY_SIZE, CHARACTER_SPEED, TILE_SIZE } from '../constrants';

/**
 * @typedef {Object} CharacterProps
 * @property {CanvasRenderingContext2D} ctx - The canvas rendering context
 * @property {number[][]} map - The base map layer
 * @property {(number | null)[][]} overlayLayer - The overlay layer
 * @property {(number | null)[][]} secondOverlayLayer - The second overlay layer
 * @property {(x: number, y: number) => boolean} isWalkable - Function to check if a position is walkable
 * @property {(x: number, y: number) => void} checkMapTransition - Function to check map transitions
 * @property {'tilemap1' | 'tilemap2' | 'tilemap3'} currentMap - The current map identifier
 */
