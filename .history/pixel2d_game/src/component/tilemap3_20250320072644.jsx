...
import { TILE_SIZE, TRANSITION_COOLDOWN, blockingBaseTiles, blockingItemIDs } from '../constants.jsx';
...
const Tilemap3 = () => {
  ...
  const lastTransitionTime = useRef(0); // Changed from let to useRef
  ...
  useEffect(() => {
    ...
    tilesetRef.current.src = "./asset/assets/tilemap1asset.png";
    ...
    const checkMapTransition = () => {
      // No transitions out of tilemap3 as per original
    };

    const { updateCharacter, drawCharacter } = Character({
      ctx, map, overlayLayer, secondOverlayLayer, isWalkable, checkMapTransition, currentMap: 'tilemap3'
    });
