...
import tilemapImage from "./asset/assets/tilemap1asset.png"; // Adjust path as needed
import spriteSheetImage from "../component/asset/assets/hero.png"; // Adjust path as needed

const Tilemap3 = ({ characterPos, onMapTransition }) => {
  const canvasRef = useRef(null);
  const TILE_SIZE = 32;
  const MAP_WIDTH = 30 * TILE_SIZE; // 960px
  const MAP_HEIGHT = 20 * TILE_SIZE; // 640px
  const CHARACTER_DISPLAY_SIZE = 96;
  const CHARACTER_SPEED = 4; // Increased to 4 for faster movement
  ...
  useEffect(() => {
    ...
    const updateCharacter = (deltaTime) => {
      ...
      if (isMoving) {
        const speed = CHARACTER_SPEED; // Simplified speed calculation
      ...
      const newX = lerp(character.x, character.targetX, deltaTime * 20); // Increased lerp speed
      const newY = lerp(character.y, character.targetY, deltaTime * 20);
    ...
    const draw = (ctx, tileset, spriteSheet) => {
      ...
      const offsetCharX = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2 * scale;
      const offsetCharY = (TILE_SIZE - CHARACTER_DISPLAY_SIZE) / 2 * scale;
      ctx.drawImage(
        spriteSheet,
        srcX,
        srcY,
