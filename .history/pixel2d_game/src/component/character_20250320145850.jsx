...
const Character = ({ initialPos, onPositionUpdate, isWalkable, scale, offsetX, offsetY }) => {
  const CHARACTER_DISPLAY_SIZE = 96;
  const CHARACTER_SPEED = 256; // Faster speed for snappy movement (pixels per second)
  const FRAME_DURATION = 100; // Faster animation for a quick feel (milliseconds)
  ...
  const render = (ctx) => {
    ...
    const offsetCharX = (32 - CHARACTER_DISPLAY_SIZE) / 2 * scale; // Assuming TILE_SIZE is 32
