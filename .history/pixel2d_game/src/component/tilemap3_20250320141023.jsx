...
const Tilemap3 = ({ characterPos, onMapTransition }) => {
  ...
  useEffect(() => {
    ...
    // Calculate scale to fit map, then center it
    const scaleX = canvas.width / MAP_WIDTH;
    const scaleY = canvas.height / MAP_HEIGHT;
    const scale = Math.min(scaleX, scaleY, 1); // Max scale of 1 to keep pixel art crisp
    ...
    const updateCharacter = (deltaTime) => {
      ...
      if (isMoving) {
        const speed = CHARACTER_SPEED * deltaTime * 60;
